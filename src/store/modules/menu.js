...
        context.state.activeItem = '/admin/index';
        context.state.tabs=[{label:'首页',index:'/admin/index'}]
        context.commit("initMenu", { menu: menumap });
      });
    } else {
      getMenu2().then(res => {
        let menumap = res.data;
        context.state.activeItem = '/user/index';
        context.state.tabs=[{label:'首页',index:'/user/index'}]
        context.commit("initMenu", { menu: menumap });
      });
    }
  },
...
      context.commit("switchTab", "/admin/index");
    } else {
      if(activeItem==="/user/index"){
      context.commit("switchTab", "/user/index");
      }else{
